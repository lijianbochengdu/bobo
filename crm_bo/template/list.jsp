...
<title>${domain}列表展示</title>
<%@include file="/WEB-INF/view/common/common.jsp"%>
<script type="text/javascript">
	$(function() {
		//声明变量
		var ${lowerDomain}Grid, ${lowerDomain}Form, ${lowerDomain}Dlg;
		//缓存变量
		${lowerDomain}Grid = $('#${lowerDomain}Grid');//数据表格
		${lowerDomain}Form = $("#${lowerDomain}Form");//新增或修改的表单
		${lowerDomain}Dlg = $("#${lowerDomain}Dlg");//会话框
		//初始化组件
		${lowerDomain}Form.form({
			url : '/${lowerDomain}/save',
			...
			success : function(result) { // result返回的数据
				try {
					...
					if (data.success) {
						${lowerDomain}Dlg.dialog('close'); // 关闭选项卡
						${lowerDomain}Grid.datagrid('reload'); // 重新加载数据
					} else {
						$.messager.show({ //失败
							title : '保存出错',
							msg : data.errorMsg
		...
		var objCrm = {
			create : function() {//新增
				${lowerDomain}Form.form('clear'); //清除原来的。
				//设置状态有默认值
			/* 	$("#radio").prop("checked", true); */
				${lowerDomain}Dlg.dialog('open').dialog('setTitle', '新增用户'); //打开一个会话框，设置标题
			},
			deleteObj : function() { //删除
				console.debug(111);
				var row = ${lowerDomain}Grid.datagrid('getSelected'); //获得选中的行的数据
				if (!row) {
				...
				if (row) {
					$.messager.confirm('Confirm', '确认删除吗?', function(r) {
						if (r) {
							$.post('/${lowerDomain}/delete/' + row.id, //这里是jQuery里面的json,所以不用转
							function(result) { //处理结果**********
								if (result.success) {
									${lowerDomain}Grid.datagrid('reload'); // 刷新数据
								} else {
									$.messager.show({ // 显示错误
										title : 'Error',
										msg : result.msg
			...
			update : function() {//修改
				var row = ${lowerDomain}Grid.datagrid('getSelected');
				if (!row) {
					$.messager.alert('温馨提示', '请选择修改的项!', 'warning');
				}
				if (row) {
					${lowerDomain}Dlg.dialog('open').dialog('setTitle', '修改用户');
					// 部门经理
					//if (row.manager) {
					...
					//}
					${lowerDomain}Form.form('load', row); //回显
				}
			},
			save : function() { //保存
				${lowerDomain}Form.submit();
			},
			cancel : function() { //取消
				${lowerDomain}Dlg.dialog('close');
			},
			refresh : function() {//刷新
				//重新载入当前页面数据  
				${lowerDomain}Grid.datagrid("reload");
			}
...
<body>
	...
	<table id="${lowerDomain}Grid" class="easyui-datagrid" title="${domain}管理"
		fit="true" url="/${lowerDomain}/json" pagination="true" rownumbers="true"
		singleSelect="true" toolbar="#${lowerDomain}tToolbar" fitColumns="true"
		border="1">
		<thead>
			<tr>
	...
	<div id="${lowerDomain}Dlg" class="easyui-dialog"
		style="width: 400px; height: 280px; padding: 10px 20px" closed="true"
		buttons="#${lowerDomain}Dlg-buttons">
		<div class="ftitle">新增用户</div>
		<!-- 里面的表格  -->
		<form id="${lowerDomain}Form" class="myform" method="post">
            ...
            idField: 'id',
            textField: 'realName', 
            url: '/${lowerDomain}/${lowerDomain}Manager',
            method: 'post',
            columns: [[
	...
	<!-- 选择框的按钮 -->
	<div id="${lowerDomain}Dlg-buttons">
		<a href="javascript:void(0)" class="easyui-linkbutton"
