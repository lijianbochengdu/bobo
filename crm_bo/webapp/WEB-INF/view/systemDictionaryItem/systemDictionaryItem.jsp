...
<title>SystemDictionaryItem列表展示</title>
	...
	$(function() {
		...
		var objCrm = {
			...
			/* 	$("#radio").prop("checked", true); */
				systemDictionaryItemDlg.dialog('open').dialog('setTitle', '新增用户'); //打开一个会话框，设置标题
			},
					...
					$.messager.confirm('Confirm', '确认删除吗?', function(r) {
						if (r) {
							...
							function(result) { //处理结果**********
								if (result.success) {
									systemDictionaryItemGrid.datagrid('reload'); // 刷新数据
								} else {
									$.messager.show({ // 显示错误
										title : 'Error',
										msg : result.msg
			...
			update : function() {//修改
				...
				if (row) {
					systemDictionaryItemDlg.dialog('open').dialog('setTitle', '修改用户');
					// 部门经理
					//if (row.manager) {
...
<body>
	...
	<table id="systemDictionaryItemGrid" class="easyui-datagrid" title="数据字典明细管理"
		fit="true" url="/systemDictionaryItem/json" pagination="true" rownumbers="true"
		singleSelect="true" toolbar="#systemDictionaryItemtToolbar" fitColumns="true"
		border="1">
		<thead>
			<tr>
	...
	<div id="systemDictionaryItemDlg" class="easyui-dialog"
		...
		<form id="systemDictionaryItemForm" class="myform" method="post">
			...
			<div class="fitem">
				<label>所属分类:</label> <input name="parentId" class="easyui-validatebox"
					required="true">
			</div>
			<div class="fitem">
