...
<script type="text/javascript">
	$(function() {
		//声明变量
		var roleGrid, roleForm, roleDlg;
		//缓存变量
		roleGrid = $('#roleGrid');//数据表格
		roleForm = $("#roleForm");//新增或修改的表单
		roleDlg = $("#roleDlg");//会话框
		//初始化组件
		roleForm.form({
			url : '/role/save',
			...
			success : function(result) { // result返回的数据
				try {
					...
					if (data.success) {
						roleDlg.dialog('close'); // 关闭选项卡
						roleGrid.datagrid('reload'); // 重新加载数据
					} else {
						$.messager.show({ //失败
							title : '保存出错',
							msg : data.errorMsg
		...
		var objCrm = {
			...
			/* 	$("#radio").prop("checked", true); */
				roleDlg.dialog('open').dialog('setTitle', '新增用户'); //打开一个会话框，设置标题
			},
			deleteObj : function() { //删除
				console.debug(111);
				var row = roleGrid.datagrid('getSelected'); //获得选中的行的数据
				if (!row) {
			...
			update : function() {//修改
				...
				if (row) {
					roleDlg.dialog('open').dialog('setTitle', '修改用户');
					// 部门经理
					//if (row.manager) {
...
<body>
	...
	<table id="roleGrid" class="easyui-datagrid" title="部门管理"
		fit="true" url="/role/json" pagination="true" rownumbers="true"
		singleSelect="true" toolbar="#roletToolbar" fitColumns="true"
		border="1">
		<thead>
			<tr>
				<th field="id" width="80">id</th>
				<th field="sn" width="80" align="center">角色编号</th>
	...
	<div id="roleDlg" class="easyui-dialog"
		...
		<!-- 里面的表格  -->
		<form id="roleForm" class="myform" method="post">
			<input name="id" type="hidden">
			<div class="fitem">
				<label>角色编号:</label><input name="sn" class="easyui-validatebox"
					required="true">
			</div>
			<div class="fitem">
